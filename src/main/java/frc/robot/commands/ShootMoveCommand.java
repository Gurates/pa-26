 ...
 */
public class ShootMoveCommand extends Command {
    ...
    public void execute() {
        ...
        Translation2d fieldVelocity = getFieldVelocity();
        double currentSpeed = fieldVelocity.getNorm();

        Translation2d relativeVelocity = fieldVelocity.rotateBy(targetHdg.unaryMinus());
        double lateralSpeed = relativeVelocity.getY();

        if (currentSpeed <= 3.0 && lateralSpeed < 0) {
            targetHdg = targetHdg.plus(Rotation2d.fromDegrees(5));
        }
        if (currentSpeed <= 3.0 && lateralSpeed > 0) {
            targetHdg = targetHdg.minus(Rotation2d.fromDegrees(5));
        }
        if (currentSpeed >= 3.0 && currentSpeed <= 5.0 && lateralSpeed < 0) {
            targetHdg = targetHdg.plus(Rotation2d.fromDegrees(10));
        }
        if(currentSpeed >= 3.0 && currentSpeed <= 5.0 && lateralSpeed > 0){
            targetHdg = targetHdg.minus(Rotation2d.fromDegrees(10));
        }
